...
/// Records tab (记事), reworked around the open-notebook model: a grid of
/// Notebooks plus quick standalone notes. Tapping a notebook opens Sources /
/// Notes / Chat.
struct NotebookView: View {
        ...
        ScrollView {
            VStack(alignment: .leading, spacing: Brand.stack) {
                SectionHeader(title: "Notebook", subtitle: "把资料、笔记、对话集中到一个主题", systemImage: "books.vertical",
                              trailing: AnyView(Button { newNotebook = true } label: { Image(systemName: "plus.circle.fill") }))
                ...
                HStack {
                    ...
                    Spacer()
                    Button { quickNote = true } label: { Image(systemName: "plus.circle") }
                }
    ...
    private func notebookCard(_ nb: Notebook) -> some View {
        VStack(alignment: .leading, spacing: 8) {
            ...
            Text(nb.name).font(.headline).lineLimit(2)
            Text("\(nb.sources.count) 资料 · \(store.notes(in: nb).count) 笔记")
                .font(.caption2).foregroundStyle(.secondary)
            Text(RelativeTime.string(nb.updatedAt)).font(.caption2).foregroundStyle(.tertiary)
        }
    ...
    private func quickNoteRow(_ note: Note) -> some View {
        VStack(alignment: .leading, spacing: 4) {
            HStack {
                ...
                Spacer()
                Text(RelativeTime.string(note.updatedAt)).font(.caption2).foregroundStyle(.tertiary)
            }
                        ...
                        HStack {
                            ForEach(emojis, id: \.self) { e in
                                Text(e).font(.title2).padding(8)
                                    .background(emoji == e ? Color.blue.opacity(0.2) : .clear, in: Circle())
