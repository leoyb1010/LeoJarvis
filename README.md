...
- **Reach 渠道检测**：`/reach/status` 会按 Agent-Reach 的渠道模型检查网页、GitHub、RSS、YouTube、B站、Exa、Twitter/X、Reddit、小红书、抖音、LinkedIn、微信公众号、微博、V2EX、雪球、小宇宙播客等来源，告诉你哪些已可用、哪些需要登录/Cookie/MCP。`/reach/read-url` 通过 Jina Reader 把网页转成正文，`/reach/github/repo` 用 `gh` 读取仓库介绍、topic、星标、语言、release 和更新时间，避免情报中心只有重复标题。
