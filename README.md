...
Cortex 支持把每台 Mac 的健康摘要汇聚成一个移动端友好的设备看板：
...
macOS 右上角菜单栏 starter 在 `menubar/`：用 Xcode 新建 SwiftUI macOS App 后粘贴 `menubar/CortexMenuBarApp.swift`，即可从 `http://127.0.0.1:8787/device/summary` 读取本机健康状态。
