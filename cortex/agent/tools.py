...
def _t_write_journal(args: dict) -> str:
    ...
    if not text:
        return "（日记内容为空，未写入）"
    eid = db.insert_event(source="journal", kind="journal", domain="life",
                          title=text[:40], content=text)
    return f"已记入日记（id={eid}）。" if eid else "（日记疑似重复，未写入）"
...
TOOLBUS.register(Tool("write_journal", "把一段内容记入个人日记（归到生活域）。",
                      {"text": "日记正文"}, _t_write_journal))
...
# Journal
TOOLBUS.register(Tool("search_journal", "检索/列出个人日记。", {"query": "关键词，留空则列最近"}, _t_search_journal))
